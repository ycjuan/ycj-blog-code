...
Data genData(Config config)
{
    ...
    // Allocate memory
    {
        ...
        CHECK_CUDA(cudaMallocHost(&data.h_centroidEmb, config.numCentroids * config.embDim * sizeof(EMB_T)));
        CHECK_CUDA(cudaMalloc(&data.d_centroidEmb, config.numCentroids * config.embDim * sizeof(EMB_T)));
    ...
    // Generate data
    {
        ...
        // Generate centroid embeddings
        {
            ...
            for (int centroidIdx = 0; centroidIdx < config.numCentroids * config.embDim; centroidIdx++) 
            {
                ...
                for (int embIdx = config.embDim; embIdx < config.embDim * 2; embIdx++)
                {
                    const auto addr = getMemAddr(centroidIdx, embIdx, config.numCentroids, config.embDim * 2);
                    data.h_centroidEmb[addr] = (EMB_T)config.stdDev;
                }
            }
        }
