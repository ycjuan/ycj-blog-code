...
void BlockManager::init(SystemConfig cfg)
{
    ...
    vv_dispatcher_.resize(cfg_.numPartitions);
    ...
    for (int p = 0; p < cfg_.numPartitions; p++)
    {
        ...
        vv_retriever_[p].reserve(cfg_.numShards);
        vv_dispatcher_[p].reserve(cfg_.numShards);
        for (int s = 0; s < cfg_.numShards; s++)
        {
            unsigned seed = (unsigned)(p * cfg_.numShards + s);
            vv_retriever_[p].emplace_back();
            vv_retriever_[p][s].init(cfg_.numDocsPerShard, cfg_.embDim, seed);

            Retriever* retriever   = &vv_retriever_[p][s];
            int        numToReturn = cfg_.numToReturn;
            vv_dispatcher_[p].push_back(std::make_unique<Dispatcher>());
            vv_dispatcher_[p][s]->start(cfg_.batchSize,
                                        cfg_.maxWait,
...
{
    for (auto& v_dispatcher : vv_dispatcher_)
    {
...
std::future<RequestResult> BlockManager::submit(Query query)
{
    ...
    for (int s = 0; s < cfg_.numShards; s++)
    {
        v_shardFuture->push_back(vv_dispatcher_[partitionId][s]->submit(query));
    }
