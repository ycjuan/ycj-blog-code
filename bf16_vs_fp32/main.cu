...
template <typename T_RST, typename T_ACC>
__global__ void kernelFp32(float *d_doc, float *d_req, T_RST *d_rst, int numDocs, int embDim)

{
    ...
    if (i < numDocs)
    {
        ...
        {
            if (accumulatorType == FP64)
                rstFP64 += d_doc[j * kNumDocs + i] * d_req[j];
            else if (accumulatorType == FP32)
                rstFP32 += d_doc[j * kNumDocs + i] * d_req[j];
        }
        d_rst[i] = rst;
    }
}

__global__ void kernelBf16(__nv_bfloat16 *d_doc, __nv_bfloat16 *d_req, float *d_rst, int numDocs, int embDim, )
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < numDocs)
    {
        ...
        for (int j = 0; j < embDim; j++)
        {
            __nv_bfloat16 docVal = d_doc[j * kNumDocs + i];
            __nv_bfloat16 reqVal = d_req[j];
            rst += docVal * reqVal;
        }
            rst += d_doc[j * kNumDocs + i] * d_req[j];
        d_rst[i] = rst;
    }
}
...
vector<float> runGpu(float *d_doc, float *d_req, float *d_rst, Meta *d_meta)
{
    ...
    for (int t = -3; t < kNumTrials; t++)
    {
        CudaTimer timer;
        timer.tic();
        kernelFp32<<<gridSize, kBlockSize>>>(d_doc, d_req, d_rst, d_meta, kNumDocs, kEmbDim, kMetaSize);
