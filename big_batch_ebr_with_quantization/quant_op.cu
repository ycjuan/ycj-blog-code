...
{    
    ...
    int MATRIX_K = data.numInt32;

    T_QUANT *a = data.d_doc;
    T_QUANT *b = data.d_req;

    T_QUANT_RST *c_wmma = data.d_rstGpu;
    ...
    int unitX = WMMA_M * blockDim.x * 4 / 32; // 4 is loop unrolling; 32 is warp size
    int unitY = WMMA_N * blockDim.y * 4;
...
#pragma omp parallel for
    for (int i = 0; i < data.numDocs; i++)
    {
        ...
        {
            T_QUANT_RST totalCount = 0;
            for (int k = 0; k < data.numInt32; k++)
            {
                T_QUANT reqVal = data.h_req[getMemAddr(j, k, data.numReqs, data.numInt32, data.reqMemLayout)];
                T_QUANT docVal = data.h_doc[getMemAddr(i, k, data.numDocs, data.numInt32, data.docMemLayout)];
                T_QUANT bitwiseRst = ~(reqVal ^ docVal);
                bitset<32> bits(bitwiseRst);
                int count = bits.count();
                totalCount += count;
            }
            data.h_rstCpu[getMemAddr(i, j, data.numDocs, data.numReqs, data.rstLayoutCpu)] = totalCount;
        }
    }
